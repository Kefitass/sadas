...
def manejar_estado_jugando(pantalla, estado_juego_param):
    ...
    global pilas_tablero, pilas_recoleccion, mazo_reserva, pila_descarte
    global sonido_activado # Asegúrate de que esta línea esté presente si usas 'sonido_activado' aquí
    ...
    for evento in pygame.event.get():
        ...
        if evento.type == pygame.MOUSEBUTTONDOWN:
            if evento.button == 1: # Clic izquierdo
                ...
                # Lógica para el click en el mazo de reserva o para reciclar el descarte
                ...
                if rect_mazo_reserva.collidepoint(mouse_x, mouse_y):
                    if mazo_reserva: # Si el mazo de reserva no está vacío, saca una carta
                        carta_descarte = mazo_reserva.pop()
                        pila_descarte.append((carta_descarte[0], carta_descarte[1], True))
                        movimientos_realizados += 1
                    else: # Si el mazo de reserva está vacío, intenta reciclar la pila de descarte
                        # Aquí llamamos a la función de reciclaje, que ahora maneja el caso de pila_descarte vacía
                        if reciclar_pila_descarte():
                            movimientos_realizados += 1 # Cuenta el reciclaje como un movimiento
                    continue # Continúa al siguiente evento
                
                #compruebo el click en la pila de descarte
                if pila_descarte:
        ...
        # ESTA LÍNEA SERÁ REEMPLAZADA POR LA VERIFICACIÓN DE VICTORIA AUTOMÁTICA
    ...
    # *** NUEVA LÓGICA DE VERIFICACIÓN DE VICTORIA AQUÍ ***
...
def reciclar_pila_descarte():
    """
    Mueve las cartas de la pila de descarte al mazo de reserva y las baraja,
    cuando el mazo de reserva está vacío.
    Ahora también proporciona feedback si no hay cartas para reciclar.
    """
    global mazo_reserva, pila_descarte # Declarado global para ser explícito

    if not mazo_reserva: # La condición principal es que el mazo de reserva esté vacío
        if pila_descarte: # Si hay cartas en la pila de descarte, las reciclamos
            cartas_a_reciclar = []
            while pila_descarte:
                carta = pila_descarte.pop() 
                cartas_a_reciclar.append((carta[0], carta[1], False)) 

            mazo_reserva.extend(cartas_a_reciclar) 
            random.shuffle(mazo_reserva) 

            print("Pila de descarte reciclada al mazo de reserva.")
            return True 
        else:
            # Si no hay cartas en la pila de descarte, informa al usuario
            print("No hay cartas en la pila de descarte para reciclar.")
            return False # No se realizó el reciclaje
    return False # El mazo de reserva no estaba vacío

# *** FUNCIÓN DE VERIFICACIÓN DE VICTORIA AÑADIDA AQUÍ ***
def verificar_condicion_victoria(): ##verifica si ganaste la partida con las 4 pilas llenas (10 cada una)
    for fundacion in pilas_recoleccion:
        if len(fundacion) != 10:
            return False # Si alguna fundación no tiene 10 cartas, no se ha ganado
    print("GANASTE EL JUEGO!!!!")
    return True # Todas las fundaciones tienen 10 cartas
